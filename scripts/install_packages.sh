...
sudo apt-get update
sudo apt-get install --upgrade -y nodejs git jq postgresql-client python3-pip gh
...
NUM_PACKAGES=$(apt list --installed \
  | grep -E "(^nodejs/|^git/|^jq/|^postgresql-client/|^python3-pip|^gh)" | wc -l)
if [[ $NUM_PACKAGES != 6 ]]; then
